...
<div class="px-6 py-8">
    ...
    @php($hasFilters = request()->filled('search') || request()->filled('bootcamp_id') || request()->filled('status') || request()->filled('start_date') || request()->filled('end_date'))
        ...
        <form method="GET" class="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input id="search" type="text" name="search" value="{{ request('search') }}"
                           placeholder="Cari batch atau bootcamp..."
                ...
                <div>
                    ...
                    <select id="bootcamp_id" name="bootcamp_id"
                        ...
                        @foreach($bootcamps as $bootcampOption)
                            <option value="{{ $bootcampOption->id }}" {{ (string) request('bootcamp_id') === (string) $bootcampOption->id ? 'selected' : '' }}>{{ $bootcampOption->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
            ...
            <div class="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div class="text-sm text-gray-500">
                    @if($hasFilters)
                        Filter batch diterapkan sesuai kriteria di atas.
                    @else
