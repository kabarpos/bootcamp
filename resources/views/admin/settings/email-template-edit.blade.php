...
<div class="px-6 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Edit Template Email</h1>
            <p class="text-gray-600">Sesuaikan subject dan isi email untuk template <strong>{{ $template->name }}</strong>.</p>
        </div>
        <a href="{{ route('admin.settings.email.edit') }}" class="text-sm text-gray-600 hover:text-gray-800">&larr; Kembali</a>
    </div>

    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Informasi Template</h2>
            <p class="mt-1 text-sm text-gray-500">Placeholder tersedia: {{ implode(', ', $template->requiredPlaceholders()) ?: '-' }}</p>
        </div>
        <div class="p-6">
            <form action="{{ route('admin.settings.email.templates.update', $template) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    <label for="content" class="block text-sm font-medium text-gray-700 mb-1">Isi Email</label>
                    ...
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-2 text-xs text-gray-500">Gunakan placeholder seperti <code>{{ '{name}' }}</code>, <code>{{ '{invoice_no}' }}</code>. Baris baru akan otomatis diberi garis baru.</p>
                </div>
                ...
                <div class="flex items-center justify-end gap-3">
                    <a href="{{ route('admin.settings.email.edit') }}" class="text-sm text-gray-600 hover:text-gray-800">Batal</a>
