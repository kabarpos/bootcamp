<div class="bg-card/80 backdrop-blur-sm border border-border rounded-lg shadow-sm overflow-hidden flex flex-col h-full {{ $featured ? 'ring-2 ring-primary' : '' }}">
    @if($featured)
    ...
    <div class="p-6 pt-0">
        <x-public.button href="{{ $buttonLink ?? '#' }}" variant="{{ $featured ? 'primary' : 'secondary' }}" class="w-full">
